...
writeRaster(palmyra,"8-28-11x11NORMwateryPalmyra.tif")
writeRaster(teraina,"8-28-11x11NORMwateryTeraina.tif")
writeRaster(fanning,"8-28-11x11NORMwateryFanning.tif")
...
rf.mdl.mask <- randomForest(x=allTrainingData[,c(3:13)], y=as.factor(allTrainingData[,"isWater"]), ntree=500, importance=TRUE, progress="window")
saveRDS(rf.mdl.mask,"8.28WATERNORMrandomForestSMPalmTerrFann.RDS")

palmyramask = predict(palmyra, rf.mdl.mask, filename="8.28-SMPalmyraWaterMask.tif", type="response", index=1, na.rm=TRUE, progress="window", overwrite=TRUE)
plot(palmyramask)
