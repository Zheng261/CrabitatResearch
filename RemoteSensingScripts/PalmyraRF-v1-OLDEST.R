...
# Import masked raster (I produced in ENVI; see notes above) image with 8 visual and 8 texture layers; make a single raster brick as we did above:
img.m <- brick("palmyra-2016-ALL-BANDS-MASKED-v1.tif")
names(img.m) <- c(paste0("T",1:8, coll=""), paste0("B",1:8, coll="")) #renames bands to shortened designations

# Assign raster values to training data
dataSet <- as.data.frame(extract(img.m, trainingData))
trainingData@data = data.frame(trainingData@data, dataSet[match(rownames(trainingData@data), rownames(dataSet)),])
View(trainingData)
...
predict(img.m, rf.mdl, filename="6.26MASKED.Palmyra-RF-classification-v3.img", type="response", index=1, na.rm=TRUE, progress="window", overwrite=TRUE)
...
img.1 <- raster("6.26MASKED.Palmyra-RF-classification-v3.img")

img.f <- focal(img.1, w=matrix(1,5,5), fun=modal)
writeRaster(img.f, filename = "6.26MASKEDPalmyra-RF-classification-v3-5x5-modal.tif", format = 'GTiff', overwrite = T)
