# Michael Burnett & Zheng Yan
# Random Forest classification of Palmyra v2
...
#Screw it, let's just classify the water
...
keeps <- c("landcover", "T5","T8","T6","B1","B5","B7")
trainingData.2 <- trainingData.2[, keeps, drop = FALSE]
View(trainingData.2)

# Run the Random Forest model using your training data
rf.mdl <- randomForest(x=trainingData.2@data[,2:7], y=as.factor(trainingData.2@data[,"landcover"]), ntree=5400, na.action=na.omit, importance=TRUE, progress="window")
...
predict(img, rf.mdl, filename="6.26WATERY.Palmyra-RF-classification-v3.img", type="response", index=1, na.rm=TRUE, progress="window", overwrite=TRUE)
...
img.1 <- raster("6.26WATERY.Palmyra-RF-classification-v3.img")

img.f <- focal(img.1, w=matrix(1,5,5), fun=modal)
writeRaster(img.f, filename = "6.26WATERYMODALPalmyra-RF-classification-v3-5x5-modal.tif", format = 'GTiff', overwrite = T)
#Note: these lines of code take forever to run and I'm not really sure why...

############################
###### ACCURACY TESTS ######
############################
# STILL NEED TO GET ALL THIS WORKING. Namely need to make test sample data into confusion matrix.
...
classed <- raster("6.26WATERYMODALPalmyra-RF-classification-v3-5x5-modal.tif") # Took 5x5 MODAL average
